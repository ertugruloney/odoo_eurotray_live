...
class Cronjob:
    ...
    @staticmethod
    def create_command(masterpswd, url, main_db, db_name, db_user, db_password, process_id, backup_location, storage_path, module_path, backup_format="zip",kwargs = {}):
        if backup_location == "remote":
            ...
            remote_pass = kwargs.get('remote_pass')
            temp_bkp_dir = kwargs.get('temp_bkp_dir')
            return "{} {} --mpswd '{}' --url {} --dbname {} --maindb {} --dbuser {} --dbpassword '{}' --processid {} --bkploc {} --path {} --backup_format {} --rhost {} --rport {} --ruser {} --rpass '{}' --temp_bkp_path {} >> {} 2>&1".format(PYTHON_ENV, module_path, masterpswd, url, db_name, main_db, db_user, db_password, process_id, backup_location, storage_path, backup_format, remote_host, remote_port, remote_user, remote_pass, temp_bkp_dir, LOG_FILE_PATH)
        else:
...
def add_cron(master_pass, url, main_db, db_name, db_user, db_password, process_id, backup_location, frequency, frequency_cycle, storage_path, module_path, backup_format="zip", backup_starting_time=None, **kwargs):
    _logger.info(locals())
    command = Cronjob.create_command(master_pass, url, main_db, db_name, db_user, db_password, process_id, backup_location, storage_path, module_path, backup_format, kwargs)
    # else:
