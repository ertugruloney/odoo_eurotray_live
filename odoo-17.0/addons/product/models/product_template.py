...
class ProductTemplate(models.Model):
    ...
    list_price = fields.Float(
        ...
        compute='_price_onchange',
        readonly=True
    )
    pricee=fields.Float('Price', default=1.0,
        digits='Product Price')
    ...
    stockdi = fields.Char(string='Stock Discount', readonly=True , compute='_price_onchange')
    ...
    @api.onchange('pricee')
    def _price_onchange(self):
        try:
            onhand =self.env['stock.change.product.qty'].sudo().search_read([], ['new_quantity'])[-1]['new_quantity']

        except:
            onhand=0
        print(onhand)
        if int(onhand)>=50 and int(onhand)<100:
            self.list_price=self.pricee*0.98
            self.stockdi='%2'
        if int(onhand)>=100:
            self.list_price = self.pricee * 0.95
            self.stockdi = '%5'
        if int(onhand)<50:
            self.list_price = self.pricee
            self.stockdi = '%0'
