...
class Lead(models.Model):
    ...
    lead_properties = fields.Properties(
        ...
        copy=True)
    file_upload = fields.Binary(string='Technical Documentation')

    fileshow = fields.Boolean("fileshow",default=True,compute='_onchange_techni')
    ...
    preordershow=fields.Boolean(string='Preordershow' ,default=True,compute='_onchange_stageid')
    afterordershow = fields.Boolean(string='afterordershow', default=True, compute='_onchange_stageid')
    afterorderinvalid = fields.Boolean(string='preorderinvalid', default=True, compute='_onchange_afterordershow')
    preorderinvalid = fields.Boolean(string='afterorderinvalid', default=True, compute='_onchange_fileshow')
    ...
    pyr = fields.Selection(string='Payment request', default='check',
                              selection=[('cash', 'Cash'),
                                         ('dd', 'Due date'), ('check', 'Check')])
    formatt = fields.Selection(string='Format',default='ef',
                              selection=[('ef', 'Eurotray Format'),
                                         ('cf', 'Customer Format')])


    trans = fields.Selection(string='Transportation ',default='fca',
                              selection=[('fca', 'Fca'),
                                         ('dap', 'Dap'), ('exw', 'Exw')])

    cmo=fields.Char(string='Contract manufacturing Offer')

    customerrs= fields.Selection(string='Customer /2023 ',default='or',
                              selection=[('or', 'Offer Report'),
                                         ('sr', 'Sales Report')])
    specialpacking=fields.Boolean(string='Special packaging')
    notes=fields.Char()

    orderno=fields.Char(string='Order no')
    siparat = fields.Datetime('Order receipt date' )
    siparac = fields.Datetime('Order opening date')
    kickoff = fields.Datetime('Kick off date')
    transor = fields.Selection(string='Transportation ',default='fca',
                              selection=[('fca', 'Fca'),
                                         ('dap', 'Dap'), ('exw', 'Exw')])
    aract = fields.Selection(string='Vehicle Type',default='tir',
                              selection=[('tir', 'Lorry'),
                                         ('ka', 'Truck'), ('kon', 'Container')])
    tarac = fields.Integer(string='Total Vehicles')

    partiel=fields.Boolean(string="Partiel")
    termint= fields.Datetime('Deadline')
    plannedd = fields.Datetime('Planned Date')
    packk=fields.Char('Packaging')
    contoll=fields.Char('Control')
    estonage=fields.Integer(string='Estimated Tonnage')
    esve=fields.Integer(string='Estimated Vehicle')
    quto = fields.Integer(string='Quantity Tolerancev')
    thicketo = fields.Integer(string='Thickness Tolerance')
    faur = fields.Boolean(string="Contract Manufacturing")
    rico=fields.Char(string='Risk control')
    bige = fields.Boolean(string="Notification Requirement")
    stre = fields.Datetime('Stock reservation Date')
    tashi= fields.Datetime('Target shipment date')
    avas = fields.Boolean(string="Advance Received ?")
    fupa = fields.Boolean(string="Full Payment Received ?")
    streett = fields.Char()
    streett2 = fields.Char()
    zipt = fields.Char(change_default=True)
    cityt = fields.Char()
    state_idt = fields.Many2one("res.country.state", string='State', ondelete='restrict', domain="[('country_id', '=?', country_id)]")
    country_idt = fields.Many2one('res.country', string='Country', ondelete='restrict')
    country_codet = fields.Char(related='country_id.code', string="Country Code")
    sales_id=fields.One2many('sale.order','crm_kart')
    _sql_constraints = [
    ...
    def _onchange_stageid(self):
        ...
        if int(self.stage_id.id)>=7:
            ...
            self.preordershow=True
        if int(self.stage_id.id)>=10:
            self.afterordershow  = False
        else:
            self.afterordershow =True
        for i in self.partner_id.child_ids:
            if i.type == 'delivery':
                self.streett=i.street
                self.streett2=i.street2
                self.zipt=i.zip
                self.country_idt=i.country_id
                self.country_codet=i.country_code
                self.state_idt=i.state_id
                self.cityt=i.city
    @api.onchange('techni')
    def _onchange_techni(self):
        if self.techni:
            self.fileshow = False
    ...
    @api.onchange('preordershow')
    def _onchange_fileshow(self):
        if self.preordershow:
            self.preorderinvalid=False
        else:
            self.preorderinvalid = True
    @api.onchange('afterordershow')
    def _onchange_afterordershow(self):
        if self.afterordershow:
